...
export function Booking() {
  ...
  function whatsapp() {
    ...
    if (quote) msg += ` (${checkIn} → ${checkOut}, ${formatEuro(quote.total)})`;
        ...
        <Reveal>
          <div className="mx-auto mt-10 max-w-4xl overflow-hidden rounded-3xl bg-white shadow-xl ring-1 ring-piedra/60">
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="rdp-brand mt-3 flex justify-center rounded-2xl border border-piedra bg-crema/30 p-3">
                <DayPicker
                  mode="range"
                  selected={range}
            ...
            {/* Barra inferior: resumen + pago */}
            <div className="bg-bosque px-6 py-5 text-crema md:px-8 md:py-6">
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                {/* Resumen */}
                <div className="min-w-0">
                  {!quote ? (
                    <p className="max-w-sm text-sm text-crema/70">{t("empty")}</p>
                  ) : (
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-1">
                      <div>
                        <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-ocre">
                          <Tag className="h-3.5 w-3.5" />
                          {t("total")}
                        </div>
                        <div className="font-serif text-3xl font-semibold leading-tight">
                          {formatEuro(quote.total)}
                        </div>
                      </div>
                      <div className="space-y-0.5 text-sm text-crema/80">
                        <div className="flex items-center gap-1.5">
                          <CalendarDays className="h-4 w-4" />
                          {t("nights", { count: quote.nights })}
                        </div>
                        {quote.discountLabel !== "none" && (
                          <div className="flex items-center gap-1.5 text-ocre">
                            <BadgePercent className="h-4 w-4" />
                            {quote.discountLabel === "month" ? t("discountMonth") : t("discountWeek")}
                            <span className="text-crema/70">(−{formatEuro(quote.discountAmount)})</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {error && (
                    <p className="mt-3 rounded-xl bg-terracota/25 px-3 py-2 text-sm text-crema">{error}</p>
                  )}
                </div>
                ...
                <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                  <button
                    onClick={pay}
                    disabled={submitting}
                    className="btn-primary disabled:cursor-not-allowed disabled:opacity-60 sm:px-7"
                  >
                    {submitting ? (
                      <>
                  ...
                  <button onClick={whatsapp} className="btn-light">
              ...
              <p className="mt-4 flex items-center gap-1.5 text-xs text-crema/60">
